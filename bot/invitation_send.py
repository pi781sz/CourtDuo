...
No *invitation* is written here. CLAUDE.md requires the "po akceptacji nie
można zmienić partnera" warning to be shown *before* the player confirms —
a confirmed match cannot be cancelled by either side — so this module only
shows what is about to happen and hands the tap to
bot.handlers.invitations, which runs the send transaction in
bot.invitation_engine. The one exception is send_not_on_courtduo_response,
which does write a pending_external_invites row (build order step 9) —
there is no confirmation screen in that branch for a later tap to run a
transaction from.
...
async def send_not_on_courtduo_response(
    message: Message,
    ...
    Build order step 9, PART 1: this is also the one place both of this
    module's callers reach whenever the named player has no account, so it
    is where the attempt is remembered for the "they joined" notification
    (CLAUDE.md scenario 2) -- every one of step 6's checks has already
    passed by the time either caller gets here, so the invitation this
    remembers really would have been valid.
    """
    await crud.create_pending_external_invite_if_missing(session, account.pzt_id, invitee.pzt_id, tournament.guid)
...
async def start_invitation_send(
    ...
    if invitee_account is None:
        ...
        # there is nowhere to deliver an invitation yet -- say so, remember
        # the attempt (CLAUDE.md scenario 2; build order step 9), and let
        # them type another name rather than show a confirmation screen
        # that cannot lead anywhere. The player stays in
        # PartnerSelection.waiting_name.
        await send_not_on_courtduo_response(message, session, account, tournament, invitee, lang, bot)
        return
